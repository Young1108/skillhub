...
> 目标仓库的目录约定不一样（有的用 `skills/<name>/`，有的把 skill 目录直接放仓库根），拷回去前先 `ls` 一眼，别按习惯硬套。提交时用 `git -C <仓库根> add -A` 显式指定仓库根，**不要用 `cd $(dirname <skill目录>)`** —— 当 skill 目录就在仓库根时，dirname 会落到仓库外面，`git add` 报 `not a repository` 而复制已经发生，容易出现「文件已改但没提交」的静默漏推。推完用 `git -C <仓库根> status -sb` 确认 `main...origin/main` 无领先提交。
